...
function es_get_the_formatted_field( $field, $formatter, $value = null ) {
    ...
    switch( $formatter ) {
        ...
        case 'locality':
            if ( $component = ES_Address_Components::get_property_component( get_the_ID(), $formatter ) ) {
...
function es_get_the_content( $post = 0, $more_link_text = null, $strip_teaser = false ) {
    ...
    return $elementor_editor_mode ? $property->alternative_description : get_the_content( $more_link_text, $strip_teaser, $post );
}
...
function es_get_default_thumbnail( $size = 'thumbnail' ) {
    $attachment_id = es_get_default_thumbnail_id();
