	...
	public function single_map_tab() {
		...
		if ( ( ! empty( $es_property->latitude ) && ! empty( $es_settings->google_api_key ) ) || $content ) : ?>
            <div class="es-tabbed-item es-map" id="es-map">
                ...
                <?php if ( empty( $es_settings->hide_map ) ) : ?>
...
//		wp_enqueue_script( 'es-front-single-script' );
    ...
    public function the_content( $content = null ) {
        ...
        if ( is_singular( 'properties' ) ) {
            $content = $elementor_builder ? $content : do_shortcode( '[es_single]' );
        }

//        global $post;
//
//        if ( empty( $_GET['et_fb'] ) && is_singular( Es_Property::get_post_type_name() ) && $post && $post->post_type == Es_Property::get_post_type_name() ) {
//            $elementor_builder = get_post_meta( $post->ID, '_elementor_edit_mode', true );
//            $content = $elementor_builder == 'builder' ? $content : do_shortcode( '[es_single]' );
//        }
