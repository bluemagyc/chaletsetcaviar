...
//                $query_args['order'] = 'DESC';
	    ...
	    foreach ( $addresses_fields as $address_field ) {
            if ( ! empty( $atts[ $address_field ] ) ) {
                ...
                if ( ! empty( $address_info['formatter'] ) ) {
                    if ( is_string( $atts[ $address_field ] ) ) {
                        $atts[ $address_field ] = explode( ',', $atts[ $address_field ] );
                    }
                    $query_args['meta_query'][] = array( 'key' => '_address_component_' . $address_info['formatter'], 'value' => $atts[ $address_field ], 'compare' => 'IN' );
                }
            }
        }
